...
{  
   ...
   for(int i=0;i<9;i++)
    ...
    cout<<"Source "<<src<<" -> Detination "<<destina<<" \t Distance \t Path"<<endl;
    
                  
    char snode = 65+ src;
    char node = 65 + destina; //converting from number to alphabets
    cout<<""<<snode<<"\t -> \t "<<node<<"\t\t  "<<dist[destina]<<"\t\t "<<snode;
...
void randGen(int graph[9][9], int src, int dst)
